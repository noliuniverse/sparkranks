...
let screenHeight = window.innerHeight;
  ...
  if (screenWidth < 900) {
    ...
    console.log(screenWidth, screenHeight, CHART_WIDTH);
...
var scaleY = d3.scaleLinear().domain([0, 35]).range([0, height]);
...
function setXAxis() {
    episodes.forEach(function (episode, i) {
        ...
        // Add gridline
        plot.append("path")
            .attr("d", "M" + scaleX(i) + "," + scaleY(0) + "L" + scaleX(i) + "," + scaleY(35))
            .style("opacity", "0.1")
...
function isEliminated(d) {
    return (d.ranking == "E" || d.latestRank > CUTOFF);
}
...
function getImageSource(d) {
    return "pics/" + "SquarePic" + d.nickname.replace(/ /g, "") + ".png";
}
...
function getRankInfo(d) {
    ...
    if (d.currentRank > 6) {
        return "Ranked #" + d.currentRank + " " + displayRankChange(d);
    }
    if (d.isEliminated) {
        return "Eliminated in Episode " + episodes[d.ranking.length - 1];
    }
    return "Top 6, Rank #" + d.currentRank + " " + displayRankChange(d);
}
